...
export async function GET(
  req: NextRequest,
  ctx: { params: Promise<{ ticket: string }> } // 👈 async params in Next 15
) {
  const { ticket } = await ctx.params;         // 👈 await it

  const iss = parseIssuerFromTicket(ticket);
  if (!iss) return new Response("Bad ticket", { status: 400 });

  const res = await fetch(
    `${iss.replace(/\/*$/, "")}/wp-json/epw/v1/stream?ticket=${encodeURIComponent(ticket)}`,
